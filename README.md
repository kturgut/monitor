...
For the  Heartbeat Monitoring, since all of the messages for the same sharded entity are 
in the same Kafka partition we prefer to use external shard allocation strategy for better performance.
This is possible if producer partitioning aligns with the shard extraction in cluster sharding. 

When processing heartbeats from Kafka,  all events for heartbeats satisfy the following constraints:
 * The key of the kafka message is the [heartbeat key][1] which is in turn the entity id in sharding
 * All messages for the same heartbeat end up in the same partition

[1]: processor/src/main/scala/com/brane/monitor/processor/heartbeat/Heartbeat.scala 
...
* `producer` A Kafka producer, that produces events about heartbeats for local testing 
...
* `kafka` A local Kafka server, that can help test processor backend
              
This demonstrates how the external shard allocation strategy can be used so messages are processed locally.

A Kafka broker running locally on port `9992` with a topic with 128 partitions called `heartbeat-events` for local testing 
[Kafka can be run in Docker](https://github.com/wurstmeister/kafka-docker) or run locally using the optional `kafka` project.

Please note that this is not currently serving heartbeats in packetbrain format, so for now
it can only be used to test the backend processing of lightweight heartbeat objects [TODO]


* Run the local Kafka server. This project will also create the `heartbeat-events` topic.
...
```
12:06:59.711 INFO  [run-main-0          ] s.s.embeddedkafka.KafkaBroker$        Kafka running: localhost:9992
12:06:59.711 INFO  [run-main-0          ] s.s.embeddedkafka.KafkaBroker$        Topic 'heartbeat-events' with 128 partitions created
```

If you want to use a different Kafka cluster then update the `applications.conf`s in each project to point to your 
Kafka broker if not running on `localhost:9992`.
...
```
  bin/kafka-topics.sh --create --bootstrap-server localhost:9992 --replication-factor 1 --partitions 128 --topic heartbeat-events
```
...
* If you run on different ports the first two akka remoting ports should be 2751/2752 as they are configured as seeds.
* As there is a single consumer, all partitions will initially be assigned to this node.

```
sbt "processor / run 2751 8751 8881"
```
...
```
sbt "processor / run 2752 8752 8882"
```
...
```
[info] [2020-01-16 09:59:39,923] [INFO] [akka://KafkaToSharding@127.0.0.1:2751/system/sharding/user-processingCoordinator/singleton/coordinator] - Starting rebalance for shards [45,33,16,2,3,15,11,6,36]. Current shards rebalancing: []
```
...
```
# Node 1:
curl -v localhost:8751/cluster/shards/user-processing | jq
...
```
# Node 1:
curl -s localhost:8751/cluster/shards/user-processing | jq -r "." | grep shardId | wc -l
# Node 2:
curl -s localhost:8752/cluster/shards/user-processing | jq -r "." | grep shardId | wc -l
```
...
It assumes there is one of the nodes running its front end port on 8881. 
