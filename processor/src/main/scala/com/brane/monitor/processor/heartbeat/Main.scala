...
import com.brane.monitor.heartbeat.sharding.HeartbeatServiceHandler
...
final case class ShardingStarted(region: ActorRef[HeartbeatEvents.Command]) extends Command
  ...
  def init(remotingPort: Int, akkaManagementPort: Int, frontEndPort: Int): Unit = {
    ActorSystem(Behaviors.setup[Command] {
      ctx =>
        ...
        val settings = HeartbeatProcessorSettings(KafkaToShardingProcessor, ctx.system.toClassic)
        ctx.pipeToSelf(HeartbeatEvents.init(ctx.system, settings)) {
          case Success(extractor) => ShardingStarted(extractor)
          case Failure(ex) => throw ex
        }
        starting(ctx, None, joinedCluster = false, settings)
    }, MonitorActorSystem, config(remotingPort, akkaManagementPort))

    def start(ctx: ActorContext[Command],
              region: ActorRef[HeartbeatEvents.Command],
              settings: HeartbeatProcessorSettings): Behavior[Command] = {
      import ctx.executionContext
      ctx.log.info("Sharding started and joined cluster. Starting event processor")
      val eventProcessor = ctx.spawn[Nothing](
        HeatbeatEventsKafkaProcessor(region, settings), ShardRegion )
    ...
    def starting(ctx: ActorContext[Command],
                 sharding: Option[ActorRef[HeartbeatEvents.Command]],
                 joinedCluster: Boolean,
                 settings: HeartbeatProcessorSettings): Behavior[Command] = Behaviors
      .receive[Command] {
    ...
    def running(ctx: ActorContext[Command],
                ...
                processor: ActorRef[Nothing]): Behavior[Command] =
    ...
    def startGrpc(system: ActorSystem[_],
                  frontEndPort: Int,
                  region: ActorRef[HeartbeatEvents.Command]): Future[Http.ServerBinding] = {
     val mat = Materializer.createMaterializer(system.toClassic)
     val service: HttpRequest => Future[HttpResponse] =
       HeartbeatServiceHandler(new HeartbeatGrpcService(system, region))(mat, system.toClassic)
